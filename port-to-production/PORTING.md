...
Stripe ダッシュボードの画面（2026-08-03 にオーナーが提示）では、エンドポイント
`charming-jubilee` の送信先が `https://www.mycomise.com/api/stripe-webhook` と表示されていたが、
**画面のスクリーンショットでは U+2010 と ASCII を見分けられない**（字形がほぼ同じ）。

決着させるには次のいずれか:

1. **Stripe の画面から URL の文字列をコピーして貼り付ける。** バイト単位で判別できる。
2. **エンドポイントを開いて配信履歴を見る。** 200 が並ぶ → URL は一致していて動作中
   （リネームすると壊れるので Stripe 側も同時に直すこと）。
   404 が並ぶ／履歴が空 → 発火していない（リネームが必要）。
3. **購入者にアクセスコードのメールが自動で届いているか。** 手作業で発行しているなら、
   webhook は動いていない。
