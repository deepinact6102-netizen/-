...
### webhook が一度も動いていなかったことの帰結 → **対応不要**（2026-08-03 オーナー確認済み）

一時は「既存顧客への影響」を心配したが、オーナーの回答で**全部消えた**。

> **有料の購入者はまだ一人もいない。オーナー自身の試験購入が1件あるだけ。**

したがって:

1. **コードの配布実績が無い。** 自動発行が動いていなかったこと自体に実害は出ていない。
2. **`sub:{subscriptionId}` の対応表が空でも問題ない。** 埋めるべき既存顧客がいない。
   **手作業での Redis 登録は不要。次の購入分から正しく埋まる。**
3. **リトライで届く古いイベントは破棄しないこと。** 宛先はオーナー本人だけで、
   むしろ「webhook → コード発行 → メール送信」が実際に動くかを確かめる
   **通しテストとして使う**方針（オーナー判断）。

**この節は「解決済みの経緯」として残してある。次のセッションは対応不要。**

## Stripe の送信イベント追加 → **オーナーが対応**（2026-08-03）

2026-08-03 のダッシュボード画面で、エンドポイント `charming-jubilee` の
**「リッスン対象: イベント 2 件」**を確認した（既存の2件はおそらく
`checkout.session.completed` と `customer.subscription.deleted`）。

修正4が必要とする **`customer.subscription.updated` と `invoice.payment_failed`**
は入っていなかったが、**オーナーがダッシュボードで追加する**と回答済み。

**コード側の対応は不要。** ただしデプロイ後の通しテストの前に、追加が済んでいることを
確認すること。追加されていないと修正4は永久に発火しない。
...
## 0. 課金まわりの通しテスト（オーナー実施予定・最優先）

**webhook は今まで一度も届いていないので、ここは「回帰確認」ではなく初回の実地検証になる。**
オーナーの試験購入を使って次を通す:

```
決済 → コード発行 → メール受信 → ログイン → 解約で無効化
```

前提として、**Stripe に `customer.subscription.updated` と `invoice.payment_failed` が
追加済み**であることを先に確認すること。
...
| 段 | 確認すること | 失敗した場合の疑い先 |
|---|---|---|
| 決済 → webhook 到達 | Stripe の配信履歴が **200** になる（404 が消える） | リネームがデプロイされていない |
| コード発行 | Redis に `code:{CODE}` と `sub:{subID}` の**両方**ができる | `session.subscription` が空（Payment Link の設定） |
| メール受信 | コードのメールが届く | `RESEND_API_KEY` / `MAIL_FROM` 未設定 |
| ログイン | `v5.html` にそのコードで入れる | `code:{CODE}` の `active` が `true` でない |
| 解約 | コードが失効し、`v5.html` がゲートに戻る | イベント未追加／`sub:` が空 |

解約時は `customer.subscription.updated`（status→`canceled`）と
`customer.subscription.deleted` の**両方**が飛ぶ。どちらが先に処理されても
最終状態は `active: false` になる（後から来た方は再送・順序逆転ガードで無視されるか、
同じ値を上書きするだけ）。**片方しかログに出なくても異常ではない。**

なお解約を「期間末で解約」にした場合、その時点の
`customer.subscription.updated` は status が `active` のままなので**失効しない**。
これは意図した挙動（支払い済みの期間は使える）。即時失効を見たいならサブスクを
**即時キャンセル**すること。
...
5. **Stripe（任意・テストモード）**: サブスクを `past_due` にして**アクセスが残る**こと、
   `unpaid` / `canceled` にして**失効する**こと。`past_due` で失効したら
   `ACCESS_BY_STATUS` が書き換わっている。
